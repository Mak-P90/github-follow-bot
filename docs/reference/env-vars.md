...
Tabla canónica de variables detectadas en runtime (`bot.py`).
...
Define la fuente de candidatos para `python bot.py run`.

- `followers` (default): realiza follow-back usando tus followers directos.
- `expand`: activa descubrimiento por semillas sobre tus seguidos (`/users/{self.user}/following`) y recorre por semilla dos subfases (`followers` y `following`) con cursor persistente en `settings`.

### Notas operativas de `expand`

- Cursor persistido: `expand_seed_index`, `expand_seed_login`, `expand_seed_phase`, `expand_seed_page`.
- Deduplicación: evita `self`, usuarios ya seguidos, y usuarios con follow exitoso registrado (`follow_actions.success=1`) para evitar refollow automático.
- Ejecución: no altera el worker ni la cola; reutiliza `follow_jobs` + `process_follow_queue`.
- Límite: respeta `BOT_MAX_FOLLOWS_PER_RUN`.
